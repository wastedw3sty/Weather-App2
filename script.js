...
function updateWeatherDisplay(boardId) {
    ...
    mondaySdk.api(`query { boards (ids: ${boardId}) { items { column_values { title, text } } } }`).then(res => {
        // Clear previous weather data
        document.getElementById('weather-display').innerHTML = '';
        // Process the response and update the display
        const locations = res.data.boards[0].items.map(item => {
            // Assuming 'Location' is the title of the column that contains the location data
            const locationColumn = item.column_values.find(column => column.title === 'Location');
            return locationColumn.text;
        });
